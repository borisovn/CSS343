...
int main() {
	...
	while (!infile.eof()) {
		...
		T.displaySideways();
		cout << "Test delete method" << endl;
		bool c = T.deleteNode(notND);
		cout << "check delete for data not " << c << endl;
		if (c) {
			cout << "print updated table: " << endl;
			T.displaySideways();
		}
		// test retrieve 
...
void buildTree(BinTree& T, ifstream& infile) {
	...
	for (;;) {
		...
		bool success = T.insertBinary(ptr);
		if (!success)
